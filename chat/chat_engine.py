...
# Try to import from the chat package first (when used as a module)
try:
    from chat.embedding_model import EmbeddingModel
except ImportError:
    # If that fails, try to import from the same directory (for direct script use)
    try:
...
class ZephyrChatEngine:
    ...
    def __init__(self, data_path="data/sentiment_results.json"):
        ...
        # load the comments data for reference
        ...
        # FAISS index will be stored in the same directory as the sentiment_results.json
        faiss_path = os.path.join(os.path.dirname(data_path), "faiss_index.pkl")

        # Check if index exists, if it does, use the existing one
